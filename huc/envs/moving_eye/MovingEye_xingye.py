...
class MovingEye_bi(Env):
    ...
    def __init__(self):
        ...
        self._model = mujoco.MjModel.from_xml_path(os.path.join(directory, "moving-eye_xingye.xml"))
        ...
        self.observation_space = Box(low=0, high=255,
                                     shape=(self._width_l, self._height_l, self._width_r, self._height_r, 3))  # width, height correctly set?
        ...
        self._eye_cam_l = Camera(self._context, self._model, self._data, camera_id="left_eye",
                               resolution=[self._width_l, self._height_l], maxgeom=100, dt=1 / self._action_sample_freq)
        self._eye_cam_r = Camera(self._context, self._model, self._data, camera_id="right_eye",
                               resolution=[self._width_r, self._height_r], maxgeom=100,
    ...
    def render(self, mode="rgb_array"):
        ...
        rgb_eye, _ = self._eye_cam_l.render()
        return rgb, rgb_eye
    ...
    def step(self, action):
        ...
        action[2] = self.normalise(action[2], -1, 1, *self._model.actuator_ctrlrange[2, :])
        action[3] = self.normalise(action[3], -1, 1, *self._model.actuator_ctrlrange[3, :])
        ...
        dist_l, geomid_l = self._ray_from_site(site_name="rangefinder-site")  # Left eye
        dist_r, geomid_r = self._ray_from_site(site_name="rangefinder-site")  # Right eye

        # Check for collisions, estimate reward
        reward = 0
        if geomid_l == self._target_idx and geomid_r == self._target_idx:
            # Both cameras are focusing on the target
            reward = 3
        elif geomid_l == self._target_idx or geomid_r == self._target_idx:
            # One of the cameras is focusing on the target
            reward = 1
